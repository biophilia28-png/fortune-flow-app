...
function Card({ title, color = "text-cyan-300", children }) {
  return (
...
function getBirthYear(profile) {
  return Number((profile?.birthDate || "1990").slice(0, 4)) || 1990;
}
...
function getSamjae(profile) {
  ...
  const yearBranch = getZodiacByYear(nowYear);
  ...
  if (idx === -1) {
    return {
      ...
      status: "삼재 아님",
      level: "안정",
      text: "올해는 큰 삼재 흐름은 약합니다. 다만 개인 사주 흐름에 따라 충돌 월과 무리한 결정은 따로 조심하는 것이 좋습니다.",
    };
  }
...
function getDeep(profile) {
  ...
  return {
    ...
    strong: ["분석력", "기획력", "생존력", "집중력", "설득력", "재물감각"][seed % 6],
    weak: ["성급함", "고집", "과소비", "감정 기복", "몰빵 성향", "인간관계 피로"][(seed >> 4) % 6],
  };
}
    ...
    <div className="space-y-4">
      ...
      <Section title="타고난 운명 해석" subtitle="성향 · 십성 · 12운성">
        <div className="grid gap-3 md:grid-cols-2">
          <Card title="기본 성향" color="text-cyan-300">
            이 사주는 <b className="text-white">{deep.type}</b> 성향이 강합니다.
            강점은 <b className="text-white">{deep.strong}</b>이고,
            조심할 부분은 <b className="text-white">{deep.weak}</b>입니다.
          </Card>
      ...
      <Section title="가족·인연 복 해석" subtitle="부모복 · 형제복 · 배우자운 · 자식운">
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <Card title="부모복" color="text-violet-300">
            초년에는 도움과 간섭이 함께 들어오는 흐름입니다. 독립성이 강해질수록 운이 살아납니다.
          </Card>
          <Card title="형제·지인복" color="text-cyan-300">
            가까운 사람과 득실이 함께 생깁니다. 돈거래, 보증, 감정적 약속은 피하는 것이 좋습니다.
          </Card>
          <Card title="배우자운" color="text-pink-300">
            빠른 만남보다 오래 검증된 인연이 안정적입니다. 신뢰가 쌓인 관계에서 복이 커집니다.
          </Card>
          <Card title="자식·후배운" color="text-emerald-300">
      ...
      <Section title="삼재·주의 흐름" subtitle="띠 기준 삼재 흐름 자동 계산">
        <div className="grid gap-3 md:grid-cols-3">
          <Card title="나의 띠" color="text-cyan-300">
            <span className="text-2xl font-black text-white">{samjae.zodiac}띠</span>
          </Card>
          <Card title="올해 삼재" color="text-yellow-300">
            <span className="text-2xl font-black text-white">{samjae.status}</span>
          </Card>
          <Card title="상태" color="text-pink-300">
            <span className="text-2xl font-black text-white">{samjae.level}</span>
          </Card>
        </div>
      ...
      <Section title="재물·직업·투자 흐름" subtitle="돈이 들어오는 방식과 피해야 할 패턴">
        <div className="grid gap-3 md:grid-cols-3">
          <Card title="직업운" color="text-cyan-300">
            분석, 기획, 기술, 운영, 데이터, 관리형 분야와 잘 맞습니다.
            혼자 파고드는 일과 반복 개선형 일이 유리합니다.
          </Card>
          <Card title="재물운" color="text-yellow-300">
            한 번에 크게 벌려는 방식보다 누적 수익, 반복 수익, 현금 관리가 맞습니다.
          </Card>
          <Card title="투자 주의" color="text-rose-300">
